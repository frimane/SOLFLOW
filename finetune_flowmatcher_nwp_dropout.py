...
# For a random fraction of training windows, all NWP conditioning channels are
# zero-filled using the same neutral convention as the core absent-NWP path. If
# the checkpoint uses the NWP prior, its NWP anchor is simultaneously replaced
# by the clear-sky fallback so NWP information is not leaked through the prior.

# The default dropout probability is 0.10. The source checkpoint is never
# modified; a new checkpoint and JSON sidecar are written.
...
# Site names are station-level metadata (one value per station), not one value
# per forecasting window. They must therefore not be required to have length N.
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.10,
        help="Fraction of training windows with all NWP conditioning masked.",
    )
    parser.add_argument(
        "--verbose",
        type=int,
        choices=(0, 1),
        default=1,
        help="Print epoch loss and training progress; 1 is recommended.",
    )
...
def evaluate_condition(fm: Any, windows: Mapping[str, np.ndarray],
                       indices: np.ndarray, use_nwp: bool, seed: int) -> Dict[str, Any]:
    """Evaluate one validation split with NWP present or deliberately absent."""
    indices = np.asarray(indices, dtype=np.int64)
    nwp_all = future_nwp(windows)
    nwp = None if not use_nwp or nwp_all is None else {
        key: value[indices] for key, value in nwp_all.items()
    }
    site = windows.get("site_coords")
    site = None if site is None else np.asarray(site)[indices]
    gcs = windows.get("fut_ghi_cs")
    gcs = None if gcs is None else np.asarray(gcs)[indices]
    pred = fm.predict_ensemble(
        np.asarray(windows["hist_csi"])[indices],
        np.asarray(windows["hist_zen"])[indices],
        np.asarray(windows["fut_zen"])[indices],
        np.asarray(windows["hist_mask"])[indices],
        np.asarray(windows["fut_mask"])[indices],
        fut_ghi_cs=gcs,
        fut_nwp=nwp,
        site_coords=site,
        n_ensemble=min(50, int(fm.cfg["experiment"].get("n_ensemble", 50))),
        rng=np.random.default_rng(int(seed)),
    )
    persistence = core.persistence_reference(
        np.asarray(windows["hist_csi"])[indices],
        np.asarray(windows["hist_mask"])[indices],
    ...
    return core.evaluate(
        pred,
        np.asarray(windows["fut_csi"])[indices],
        np.asarray(windows["fut_mask"])[indices],
        fm.cfg,
        K=int(fm.K), n_days=int(fm.n_days), ghi_cs=gcs,
...
def write_metadata(
    ...
    metadata = {
        ...
        "seed": int(args.seed),
        "nwp_dropout": float(args.nwp_dropout),
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if not 0.0 <= args.nwp_dropout <= 1.0:
        raise ValueError("--nwp-dropout must be between 0 and 1")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    fm.cfg["train"]["nwp_dropout"] = float(args.nwp_dropout)
    # Do not inherit a silent setting from the source checkpoint.
    fm.cfg["train"]["verbose"] = int(args.verbose)
    ...
    print(f"NWP dropout: {args.nwp_dropout:.1%}", flush=True)
    ...
    try:
        ...
        print(
            f"training settings: batch_size={fm.cfg['train']['batch_size']}, "
            f"lr={fm.cfg['train']['lr']:.3g}, "
            f"history_dropout={float(fm.cfg['train'].get('history_dropout', 0.0)):.1%}, "
            f"nwp_dropout={float(fm.cfg['train'].get('nwp_dropout', 0.0)):.1%}",
            flush=True,
        )
        fit_arrays(fm, windows, train_idx, val_idx, args.seed)
        print("validation with NWP present...", flush=True)
        with_nwp = evaluate_condition(fm, windows, val_idx, True, args.seed + 1)
        print(
            f"  NWP present: CRPS={with_nwp.get('crps_ghi', float('nan')):.5f}, "
            f"RMSE={with_nwp.get('rmse_ghi', float('nan')):.5f}, "
            f"NICEΣ={with_nwp.get('nice_sigma_ghi', float('nan')):.5f}",
            flush=True,
        )
        print("validation with NWP masked...", flush=True)
        without_nwp = evaluate_condition(fm, windows, val_idx, False, args.seed + 2)
        print(
            f"  NWP masked:  CRPS={without_nwp.get('crps_ghi', float('nan')):.5f}, "
            f"RMSE={without_nwp.get('rmse_ghi', float('nan')):.5f}, "
            f"NICEΣ={without_nwp.get('nice_sigma_ghi', float('nan')):.5f}",
            flush=True,
        )
